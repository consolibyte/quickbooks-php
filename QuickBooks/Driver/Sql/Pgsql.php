...
class QuickBooks_Driver_Sql_Pgsql extends QuickBooks_Driver_Sql
{
     ...
     *
     * @param string $dsn
     *            DSN-style connection string (i.e.: "mysql://your-mysql-username:your-mysql-password@your-mysql-host:port/your-mysql-database")
     * @param array $config
     *            options for the driver (not currently supported)
     */
     ...
     *
     * @param string $host
     *            hostname the database is located at
     * @param integer $port
     *            port the database is at
     * @param string $user
     *            for connecting
     * @param string $pass
     *            for connecting
     ...
     *            for establishing a new link to the database, FALSE to re-use an existing one
     * @param integer $client_flags
     *            connection flags (see the PHP/MySQL documentation)
     * @return boolean
     */
    ...
    protected function _generateFieldSchema($name, $def)
    {
        switch ($def[0]) {
            ...
            case QUICKBOOKS_DRIVER_SQL_SERIAL:
                $sql = '"' . strtolower($name) . '" SERIAL NOT NULL '; // AUTO_INCREMENT
            ...
            case QUICKBOOKS_DRIVER_SQL_DATETIME:
                ...
                $sql = '"' . strtolower($name) . '" timestamp without time zone ';
...
{
                $arr_sql[] = 'CREATE INDEX "' . strtolower(implode('_', $key)) . '_' . $name . '_index" ON "' . $name . '" USING btree ("' . strtolower(implode('", "', $key)) . '")';
            } else {
                $arr_sql[] = 'CREATE INDEX "' . strtolower($key) . '_' . $name . '_index" ON "' . $name . '" USING btree ("' . strtolower($key) . '")';
            }
        }
