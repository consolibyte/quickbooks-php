    ...
    */
    ...
    protected function _IDS_v2($Context, $realmID, $resource, $optype, $xml, $ID)
    {
        ...
        if ($resource == QuickBooks_IPP_IDS::RESOURCE_COMPANY or // QuickBooks desktop
            $resource == QuickBooks_IPP_IDS::RESOURCE_COMPANYMETADATA
        )    // QuickBooks online
        {
            $post = false;
            $xml = '';
     ...
     * @return mixed
     */
    public function JSONRequest($Context, $realm, $optype, $request)
    {
        return $this->_IDS_v3Json($Context, $realm, $optype, $request);
    }
     ...
     * @return mixed
     */
    protected function _IDS_v3Json($Context, $realm, $optype, $request)
    {
        $this->baseURL(QuickBooks_IPP_IDS::URL_V3);
        // If we're in sandbox mode, use the sandbox URL instead
        if ($this->sandbox()) {
        ...
        $query = null;
        $url = $this->baseURL();
        if ($optype == QuickBooks_IPP_IDS::OPTYPE_EXCHANGERATE) {
            $post = true;
            $url .= '/company/' . $realm . '/exchangerate';
        }

        $response = $this->_request($Context, QuickBooks_IPP::REQUEST_IDS, $url, $optype, json_encode($request), $post);
        //print('URL is [' . $url . ']');
        //die('RESPONSE IS [' . $response . ']');

        // Check for generic IPP errors and HTTP errors
        if ($this->_hasErrors($response) || strpos($response, "Error") !== false) {
            return false;
        }
    ...
    protected function _request($Context, $type, $url, $action, $data, $post = true)
    {
        ...
        if ($Context->IPP()->version() == QuickBooks_IPP_IDS::VERSION_3) {
            ...
            } elseif ($action == QuickBooks_IPP_IDS::OPTYPE_EXCHANGERATE) {
        ...
        //  string(290) "OAuth realm="", oauth_signature_method="HMAC-SHA1", oauth_signature="VWvvgqXHUWQESRtweX2ZT4PXiDI%3D",  oauth_nonce="27Gyq", oauth_timestamp="1489068719", oauth_token="qyprdEzrdWijMnXR66bzm8daugZptlwcCCQTHf72HXyqFrAn", oauth_consumer_key="qyprdiMJt3f8fDtK1veHzRJyCzWkib", oauth_version="1.0""
